...
define(['pumascript'], function(puma) {
    ...
    test("Use Strict global code", function (assert) {
        ...
        result.makeValue();
        assert.ok(result.success && 4 === result.value, "Passed!");
    });
    ...
    test("use strict function code", function (assert) {
		...
		result.makeValue();
        assert.ok(result.success && 'strict mode into function' === result.value, "Passed!");
    });
    ...
    QUnit.skip("Use strict eval code", function(assert){
  	    ...
  	    assert.ok(result.success && "2 + 2" === result.value, "Passed!");
    });
    ...
    QUnit.skip("with function", function(assert){
  	    ...
  	    result.makeValue();
  	    assert.ok(result.success && 10 === result.value, "Passed!");
     });
});
